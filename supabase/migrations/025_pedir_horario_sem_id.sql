-- Pedido público SEM id de serviço (o 49 não vai mais para o Postgres).
-- Cole INTEIRO no SQL Editor e rode. Depois precisa do deploy do site.

ALTER TABLE public.appointments ALTER COLUMN service_id DROP NOT NULL;

DROP FUNCTION IF EXISTS public.pedir_horario(uuid, timestamptz, text, text, text, integer, numeric, text);

CREATE FUNCTION public.pedir_horario(
  p_salon uuid,
...
BEGIN
  ...
  v_dur := COALESCE(NULLIF(p_duracao, 0), 60);
  v_end := p_quando + (v_dur || ' minutes')::interval;
  ...
  WHERE a.user_id = p_salon
    ...
    AND tstzrange(a.start_time, a.start_time + (v_dur || ' minutes')::interval, '[)')
...
GRANT EXECUTE ON FUNCTION public.pedir_horario(uuid, timestamptz, text, text, text, integer, numeric, text) TO anon, authenticated;
