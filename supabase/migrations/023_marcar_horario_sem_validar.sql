-- Recria marcar_horario_site SEM chamar validar_horario_agendamento
-- (o join antigo services.id = appointments.service_id ainda explode uuid:"47").
-- Cole INTEIRO no SQL Editor e rode. O site já chama este RPC; não precisa esperar a Vercel.
...
BEGIN
  ...
  FOR v_row IN
    SELECT s.id, s.name, s.default_price, s.duration_minutes
    FROM services s
