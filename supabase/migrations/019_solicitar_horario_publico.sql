-- Função NOVA (nome diferente) para o site não cair no RPC antigo com p_service_id uuid.
-- Cole este arquivo INTEIRO no SQL Editor e rode. Não precisa apagar a função velha antes.

CREATE OR REPLACE FUNCTION public.solicitar_horario_publico(
  p_user_id uuid,
  p_service_id text,
